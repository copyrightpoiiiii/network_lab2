...
void SelectRdtReceiver::~RdtReceiver () {
	while (!waitAckPkt.empty ()) {
		delete waitAckPkt.top ();
		waitAckPkt.pop ();
	}
}
...
void SelectRdtReceiver::receive (const struct Packet &packet) {
	...
	if (checkSum == packet.checksum && base <= packet.seqnum && packet.seqnum < rBase) {
		...
		sort_Packet *msg = new sort_Packet;
...
lastAckPkt.
acknum = base; //确认序号等于收到的报文序号
lastAckPkt.
checksum = pUtils->calculateCheckSum (lastAckPkt);
pUtils->printPacket("接收方重新发送上次的确认报文", lastAckPkt);
pns->
sendToNetworkLayer(SENDER, lastAckPkt
);
}
